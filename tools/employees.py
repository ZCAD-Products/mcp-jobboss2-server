...
def register_employee_tools(mcp: FastMCP, client: JobBOSS2Client):
    ...
    @mcp.tool()
    async def create_attendance_ticket(employeeCode: int, ticketDate: str, **kwargs) -> Dict[str, Any]:
        """Create a new attendance ticket in JobBOSS2."""
        data = {"employeeCode": employeeCode, "ticketDate": ticketDate, **kwargs}
        return await client.api_call("POST", "attendance-tickets", data=data)
    ...
    async def create_attendance_ticket_detail(
        ...
        **kwargs
        ...
        return await client.api_call("POST", f"attendance-tickets/{ticketDate}/employees/{employeeCode}/attendance-ticket-details", data=data)

    @mcp.tool()
    async def update_attendance_ticket_detail(id: int, **kwargs) -> Dict[str, Any]:
        """Update an existing attendance ticket detail (clock in/out times)."""
        await client.api_call("PATCH", f"attendance-ticket-details/{id}", data=kwargs)
        return {"success": True}
    ...
    @mcp.tool()
    async def get_salespersons(**kwargs) -> List[Dict[str, Any]]:
        """Retrieve salesperson master records."""
        return await client.api_call("GET", "salespersons", params=kwargs)

    @mcp.tool()
    async def get_time_ticket_details(**kwargs) -> List[Dict[str, Any]]:
        """Retrieve shop floor time ticket detail entries."""
        return await client.api_call("GET", "time-ticket-details", params=kwargs)
    ...
    @mcp.tool()
    async def get_time_tickets(**kwargs) -> List[Dict[str, Any]]:
        """Retrieve time ticket headers."""
        return await client.api_call("GET", "time-tickets", params=kwargs)
